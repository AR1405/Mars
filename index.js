const API_KEY = qb5YzufYomV4hSNBtJNMJOx8klWGdO5Eu5KcDTwy;
...
let selectedSolIndex // makes global variable

getWeather().then(sols => {
	selectedSolIndex = sols.length - 1 // recent day
...
function displaySelectedSol(sols) {
	const selectedSol = sols[selectedSolIndex]  // all data for recent days
...
function displaySpeed(speed) {
 ...
 function getWeather() {
 	return fetch(API_URL) // fetch data from nasa API
		...
		.then(data => { 
			...
			const {
				sol_keys,
				validity_checks,
				...solData  // all data of different days
			} = data
			return Object.entries(solData).map(([sol, data]) => { // allow to loop through object and pass  to sol data map convert one set of information to anoither set of information
				return {
					sol: sol,
					maxTemp: data.AT.mx,
					minTemp: data.AT.mn,
